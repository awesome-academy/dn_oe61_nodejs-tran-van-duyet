...
export class ReportService {
  ...
  async update(id: number, updateReportDto: UpdateReportDto, message:string): Promise<FeedbackReport | null> {
  ...
  async remove(id: number, message: string): Promise<FeedbackReport> {
    const reports = await this.feedbackReportsRepository.findOne({ where: { id } }); 
    if (!reports) {
      throw new NotFoundException(message);
    }
